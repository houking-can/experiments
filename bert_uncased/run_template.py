...
flags.DEFINE_integer("predict_batch_size", 1, "Total batch size for pred.")
...
flags.DEFINE_integer("save_checkpoints_steps", 400,
                     "How often to save the model checkpoint.")

flags.DEFINE_integer("iterations_per_loop", 400,
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, template=None):
            ...
            for line in reader:
                if len(line) > 3:
                    ...
                    for i in range(len(tmp["abstract_text"])):
                        text_a = tmp["abstract_text"][i]
                        text_a_template = tmp["abstract_template"][i].split(' ')
                        text_a_template = [each.lower() for each in text_a_template]
                        tokens = tokenizer.tokenize(text_a)
                        text_a = text_a.split(' ')
                        label = [0 for _ in range(len(tokens))]
                        tmp_label = [0 for _ in range(len(text_a_template))]
                        j = 0
                        for k in range(len(tokens)):
                            if tokens[k] in [',','.',';','?',':','!','&']:
                                ...
                                continue
                            try:
                                index = text_a_template.index(tokens[k],j,k+1)
                                j=index+1
                                label[k]=1
                                tmp_label[index]=1
                            except:
                        ...
                        j=0
                        for k in range(len(text_a_template)):
                            if text_a_template[k]!='xxx' and tmp_label[k]==0:
                                for m in range(j,len(tokens)):
                                    x = tokens[m].replace('#','')
                                    if x in text_a_template[k]:
                                        label[m]=1
                                        j+=1

                        label.insert(0, 0)
                        lines.append((tmp["abstract_text"][i], label))
            return lines
            ...
            with tf.gfile.GFile(output_eval_file, "a") as writer:
                ...
                for key in sorted(eval_result.keys()):
                    ...
                    if key == 'eval_accuracy':
                        if eval_result[key] <= accuracy:
                            early_stop = True
                        else:
                            accuracy = eval_result[key]
    ...
    if FLAGS.do_predict:
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                ...
                probabilities = prediction["values"]
